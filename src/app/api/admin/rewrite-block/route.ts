...
export async function POST(request: NextRequest) {
  try {
    const { currentText, headlineText, hasHeadline, fullContext } = await request.json();

    if (!currentText && !headlineText) {
    ...
    let systemPrompt = `You are a skilled editor for a GenX nostalgia platform called "Best of GenX". 
...
Do NOT include any other text, explanations, or formatting.`;
    ...
    let userPrompt = `Here is the full article context for reference:
---
${fullContext}
---
